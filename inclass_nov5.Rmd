...
```{r}
dll_data_Dll <- subset(dll_data, genotype == "Dll", drop = TRUE)
dim(dll_data_Dll)
with(dll_data, table(genotype))
...
`sub()` (first match only)and `gsub()` (match pattern in global)are functions that perform replacements for the first match or globally (g) for all matches respectively. If we go back to the names of the original lines, we will see they all have `line-` in common, so perhaps we can search and replace based on that pattern. This is quite easy as well. use `gsub` to extract just the line name without the first part of the string, "line-"
...
```{r}
temp_as_factor <- dll_data$temp(factor(temp,
...
```{r}
temp_as_factor2 <- with(dll_data, if else(temp == 25, "lowtemmp", NA)
...
```
This just sorts the vector, which does not help us much. Instead we are going to use the `order()` function which provides information on the index for each observation. So (this sorts the index)
