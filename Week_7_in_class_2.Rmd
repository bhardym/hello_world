...
x does not exist outside of the second function (variable doesn't have an assignment), so we get an error message when we try to print x 
...
The other function runs faster because pre-allocating 10000 makes the function run the for loop 10000 times before moving on to the next number
...
system.time(square(x))
```


4. What is a very "`R`" way to make this (question 2 and 3) even faster? (i.e. think about vectorization)

```{r}
x = as.vector(1:10000)
system.time(print(x^2))
...
Loops are slow because it needs to go through the entire loop with every number (recursive), whereas the vector just goes through each number once.
...
n = 10000 time = 0.035
n = 100000, time = 0.331, 0.328  0.329 0.333 0.417 
